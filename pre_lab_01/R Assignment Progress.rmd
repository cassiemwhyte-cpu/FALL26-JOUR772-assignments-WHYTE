...
In the space below, write the data type for the "id" column and the "seats" column (it's in light gray, offset by \< \>). **Answer: for "id" column=\<chr\> (character); for "seats" column=\<dbl\> (**double-precision floating-point number)
...
In the space below, write the name of the business from the first row of the "name" column. **Answer: I am not seeing a name column. Only "id," "title," "description," "term," "department," "sections," "instructors," "seats," and "syllabus count"**
...
**Answer: the codeblock is 1. opening the umd courses dataset, 2. grouping the courses by term, and 3. summarizing each term by its number of classes offereed**
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer: the number of classes per each term is now listed chronologically, by most to least or "descending"**
...
**Answer: it is now arranged in accordance with the department the course is listed under; insertion of "department" along with "term" caused it to sort along both variables**
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer: now we are looking at the total seats offered each term across classes. So now we have a third column for seats offered**
